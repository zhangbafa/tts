<template>
  ...
  >
            ...
            <div class="text-xs text-gray-500 mx-10">
              剩余字数：
...
const download_filename = ref('')
const handleTextToSpeechPart=async (event:Event,index:number,content_index:number)=>{
  if(blockList.value[index].voice.shortName==''){
    toast.add({ title: "该段落未设置语音",color:'red' });
    return false;
  }
  const filename_sub = blockList.value[index].content[content_index].substring(0, 10);
  download_filename.value = `${blockList.value[index].voice.localName}-${filename_sub}`
  part.value=true
...
const handleTextToSpeech = async () => {
  ...
  if (voice.length) {
    toast.add({ title: "请为段落选择语音",color:'red' });
    return false;
  }
  loading.value = true
...
const handleDownload = ()=>{
  ...
  //   const filename_sub = blockList.value[0].content[0].substring(0, 10);
  //   download_filename.value = `${blockList.value[0].voice.localName}-${filename_sub}`
  // }
  download(3,audio_src.value,download_filename.value)
  toast.add({title:'下载成功',color:'green'})
}
const handlePreview = ()=>{
    isOpen.value = true
}
