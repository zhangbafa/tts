<template>
    <UPopover>
      <UButton color="white" :label="`语速(${speed==0?'默认值':speed})`" style="min-width: 90px;"/>
      <template #panel>
        <div class="p-4 w-72 text-center">
            <div class="flex justify-start items-centert mt-4">
                <UButton label="-" @click="handledecr" size="2xs"></UButton>
                <URange v-model="speed" :min="-100" :max="300" :step="1" class="mx-4 mr-4" @change="handleChange"/>
