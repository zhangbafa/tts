<template>
  <UPopover>
    <UButton color="white" label="插入停顿" trailing-icon="i-heroicons-chevron-down-20-solid" />

    <template #panel>
      <div class="p-4 w-72 text-center">
          <div class="grid grid-cols-2 gap-2 mb-6">
            <UButton :label="item.label" @click="handleSelect(item)" block color="white" class="" v-for="item in breakoption" :key="item"></UButton>
          </div>
          ...
          <div class="flex justify-start items-center">
              ...
              <URange v-model="volume" :min="0" :max="5000" :step="10" class="mx-4 mr-4"/>
              <UButton label="+" @click="handleincr" size="2xs"></UButton>
          </div>
          <UDivider class="mt-5"/>
          <div class="mt-4 flex justify-between items-center">
          
          <ULink @click="handleResetDefault" class="text-sm text-gray-800">恢复默认值</ULink>
          <UButton @click="handleComfirm" class="text-center">确认选择</UButton>
          </div>
      </div>
    </template>
  </UPopover>
